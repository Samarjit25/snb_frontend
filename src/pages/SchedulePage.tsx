...
import style from '../styles/SchedulePage.module.scss';
...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className={style.dashboard}>
        ...
        {classesForDay.length === 0 ? (
          ...
          classesForDay.map((cls, idx) => (
            ...
            <div key={idx} className={style.classCard}>
              ...
              <button onClick={() => navigate(`/class/${cls.subject}`)} className={style.viewBtn}>
                View class 
              </button>
              <button onClick={() => removeClass(cls)} className={style.deleteBtn}>
                Delete 
              </button>
            </div>
          ))
      ...
      <button onClick={() => navigate('/addclass')} className={style.addButton}>
        Add New Class
      </button>
    </div>
  );
